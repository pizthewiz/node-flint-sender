...
var FILE_EXTENSION_RE = new RegExp(/mp4|webm|mkv|m4a|mp3$/i);
var CONTENT_TYPES = {
  mp4: 'video/mp4',
  webm: 'video/webm',
...
function RemoteMediaPlayer(channel) {
  ...
  self.channel.on('message', function (data) {
    // perform weird double parse https://github.com/openflint/flint-player/issues/1
...
function contentTypeForFilepath(filepath) {
  var matches = filepath.toLowerCase().match(FILE_EXTENSION_RE);
  if (!matches) {
    console.error('ERROR - unrecognized file extension \'%s\'', path.extname(filepath));
    return '';
  }

  var fileExtension = matches[0];
...
RemoteMediaPlayer.prototype.likelySupportsDisplay = function (filepath) {
  return filepath.match(FILE_EXTENSION_RE) !== null;
}
